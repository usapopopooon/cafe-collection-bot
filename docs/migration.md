...
5. 新Botで `/cafe panel`、`/cafe ledger`、`/cafe ranking` を実行し、各表示先を設定する。
   チャンネル指定を省略した場合は、実行したチャンネルが表示先になる。
...
8. 公開APIの移行後にだけ、旧API側の `CAFE_COLLECTION_PUBLIC_API_ENABLED=false` を設定する。
...
両Botのパネルは併用できます。ボタン識別子はBotごとに分離し、抽選は共通APIへ
